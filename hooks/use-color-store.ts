...
/**
 * Creates a store for managing colors in the application.
 *
 * It provides several utilities for working with colors:
 * - `availableColors`: an array of objects with `name` and `root`/`dark` CSS
 *   variables.
 * - `cssColors`: an object with the currently selected color's CSS variables.
 * - `color`: the currently selected color object.
 * - `getColor()`: a function that returns the currently selected color object.
 * - `setColor(name, isUserColor)`: sets the default or user color.
 * - `updateCssVariables()`: updates the CSS variables on the document's root.
 *
 * The store is persisted in local storage.
 *
 * @param {string} [theme=light] The theme of the application.
 * @returns {object} The color store utilities.
 */
